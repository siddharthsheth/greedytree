 ...
 */
#ifdef DISPLAY
static std::ofstream display_stream("display.log", std::ios::app);
// #  define display_log(x) display_stream << x << std::endl
#  define display_log(x) std::cout << x << std::endl
#else
...
//     inline double compare_dist(const Point& other) const{
